import sys
sys.path.append("..")
import lib.utils as share_utils
...
from lib.neural_flow_components.latent_ode_lib.encoder_decoder import *
from lib.neural_flow_components.latent_ode_lib.likelihood_eval import *
...
class VAE_Baseline(nn.Module):
	...
	def compute_all_losses(self, batch_dict, n_traj_samples = 1, kl_coef = 1.):
		...
		# print(1111, batch_dict['tp_to_predict'].shape,
		# 	batch_dict['observed_data'].shape, batch_dict['observed_tp'].shape,
		# 	batch_dict['observed_mask'].shape)
		# print(batch_dict['tp_to_predict'][:2])
		# print(batch_dict['observed_tp'][:2])
		...
		if ('labels' in batch_dict) and (batch_dict['labels'] is not None) and self.use_binary_classif:
	...
	def evaluation(self, dataloader, n_batches, n_traj_samples = 1, kl_coef = 1.):
		...
		for _ in range(n_batches):
			batch_dict = share_utils.get_next_batch(dataloader)
			# bs = batch_dict["observed_data"].shape[0]
			# print(batch_dict["observed_data"].shape)

			# pred_y = self.forecasting(batch_dict["tp_to_predict"], 
			# 	batch_dict["observed_data"], batch_dict["observed_tp"], 
			...
			# print(batch_dict["data_to_predict"].sum(), batch_dict["mask_predicted_data"].sum())
