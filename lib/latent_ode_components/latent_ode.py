...
from lib.latent_ode_components.encoder_decoder import *

from torch.distributions.multivariate_normal import MultivariateNormal
from torch.distributions.normal import Normal
from torch.distributions import kl_divergence, Independent
from lib.latent_ode_components.base_models import VAE_Baseline
...
class LatentODE(VAE_Baseline):
	def __init__(self, input_dim, latent_dim, encoder_z0, decoder, diffeq_solver, 
		...
		self.diffeq_solver = diffeq_solver.to(device)
	...
	def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, 
		...
		# time_steps_to_predict = time_steps_to_predict.squeeze(dim=0)
		# truth_time_steps = truth_time_steps.squeeze(dim=0)
		# print(time_steps_to_predict.shape, truth_time_steps.shape)
		...
		if isinstance(self.encoder_z0, Encoder_z0_ODE_RNN) or \
			...
			# print(1111, truth_w_mask.shape, truth_time_steps.shape)
		...
		# print(2, first_point_enc_aug.device, time_steps_to_predict.device)
		# print(1111,((first_point_enc_aug[:-1]-first_point_enc_aug[1:])>=0).sum())
		# print(2222,((time_steps_to_predict[:-1]-time_steps_to_predict[1:])>=0).sum())

		# right before you call the solver in get_reconstruction:
		func = self.diffeq_solver.ode_func
		# if this attribute exists, move it to the right device:
		if hasattr(func, "_neginf"):
			func._neginf = func._neginf.to(time_steps_to_predict.device)

		# raise Exception(time_steps_to_predict)
		try:
			sol_y = self.diffeq_solver(first_point_enc_aug, time_steps_to_predict)
		except Exception as e:
			print("Error in ODE solver:", e)
