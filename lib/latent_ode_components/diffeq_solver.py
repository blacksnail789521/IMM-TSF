...
class DiffeqSolver(nn.Module):
	...
	def forward(self, first_point, time_steps_to_predict, backwards = False):
		...
		# print(1, first_point.device, time_steps_to_predict.device)
		# print(first_point.shape, time_steps_to_predict.shape)
		# pred_y = odeint(self.ode_func, first_point.to(self.device), time_steps_to_predict.to(self.device), 
		# 	rtol=self.odeint_rtol, atol=self.odeint_atol, method = self.ode_method)
		pred_y = odeint(self.ode_func,
                first_point.to(self.device),
                time_steps_to_predict.to(self.device),
                method="rk4")   # <— fixed‐step RK4
	...
	def sample_traj_from_prior(self, starting_point_enc, time_steps_to_predict, 
		...
		# print(2, starting_point_enc.device, time_steps_to_predict.device)
