...
class TTF_T2V_XAttn(nn.Module):
    ...
    def forward(
        ...
        # 3) Time2Vec on deltas: [B,N_max,T_f,d_tau]
        delta = (t_hat[:, None, :] - tau[:, :, None]).clamp_min(0)
        phi = self.time2vec(delta.unsqueeze(-1))

        # 4) Build KV tensor: [B,T_f,N_max,d_txt+d_tau]
        B, N_max, _ = V.shape
        _, T_f = t_hat.shape
        V_exp = V.unsqueeze(2).expand(-1, -1, T_f, -1)
        KV = torch.cat([V_exp, phi], dim=-1)
        KV = KV.permute(0, 2, 1, 3).reshape(B * T_f, N_max, self.d_txt + self.d_tau)
        KV_proj = self.KV_proj(KV)

        # 5) Prepare queries: [B*T_f,1,d_txt]
        Q = self.Q_param.expand(B, T_f, self.d_txt).reshape(B * T_f, 1, self.d_txt)

        # 6) Mask padding: [B*T_f, N_max]
        mask_pad_flat = (~note_mask).repeat_interleave(T_f, dim=0)

        # 7) Multi-head cross-attention per time step
        ...
        E_attn = attn_out.reshape(B, T_f, self.d_txt)

        # ----- NEW: override no-note samples -----
        # M_txt: [B, 1] → expand to [B, T_f, d_txt]
        mask = M_txt.view(B, 1, 1).expand(B, T_f, self.d_txt)
        ...
        Q2 = self.Q_param.expand(B, T_f, self.d_txt)
